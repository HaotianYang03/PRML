...
def extract_features_rgb(image_tensor):
    """
    从PyTorch图像张量中提取SIFT、LBP和颜色特征（融合三个通道的特征向量）
    Args:
        image_tensor: PyTorch张量，形状 (C, H, W)（C=3为RGB）
    Returns:
        fused_feature: 融合SIFT、LBP和颜色特征后的特征向量（一维数组）
    """
    ...
    # 融合SIFT、LBP和颜色特征
    fused_feature = np.concatenate([sift_feature_fused, lbp_feature_fused, color_feature])
...
def load_and_extract_features(loader):
    """
    从数据加载器中提取所有样本的SIFT、LBP和颜色特征和标签
    Args:
        loader: 数据加载器（train_loader或test_loader）
    Returns:
        features: 所有样本的SIFT、LBP和颜色特征（二维数组，形状 (n_samples, feature_dim)）
        labels: 所有样本的标签（一维数组）
    """
    features = []
    labels = []
...
if __name__ == "__main__":
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(X)):
        ...
        # 按要求保留三位小数
        print(f"Validation accuracy on fold {fold + 1}: {fold_accuracy:.4f}")
    ...
    print(f"5-fold Accuracies: {[f'{acc:.4f}' for acc in fold_accuracies]}")
